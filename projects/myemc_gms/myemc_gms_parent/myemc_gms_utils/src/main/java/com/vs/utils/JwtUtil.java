...
public class JwtUtil {
    ...
    private static SecretKey secretKeyRefresh() {
        ...
        RedisUtil.setString(SECRET_KEY_PREFIX, encodedKey, SECRET_KEY_EXPIRE_MILLISECONDS, TimeUnit.MILLISECONDS);
    ...
    private static Key keyFetchRemote() {
        ...
        String encodedKey = RedisUtil.queryString(SECRET_KEY_PREFIX);
    ...
    public static String jwtGen(Map<String, Object> payload) throws RuntimeException {
        ...
        RedisUtil.setObject(tokenKey, jwt, JWT_EXPIRE_DURATION_MINUTES, TimeUnit.MINUTES);
        return jwt;
    }
    ...
    public static void jwtParseRefresh(String jwt) {
        ...
        String token = RedisUtil.queryString(JWT_PREFIX + username);
        if(!jwt.equals(token)) throw new CustomException.InvalidTokenException("token缓存过期，请重新登陆");
        // 刷新token有效期
        RedisUtil.setObject(JWT_PREFIX + username, jwt, JWT_EXPIRE_DURATION_MINUTES, TimeUnit.MINUTES);
    }
}
