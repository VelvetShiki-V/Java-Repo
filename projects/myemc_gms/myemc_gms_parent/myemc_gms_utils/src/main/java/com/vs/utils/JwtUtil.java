...
public class JwtUtil {
    ...
    public static void jwtParseRefresh(String jwt) {
        ...
        String token = (String) RedisUtil.query(JWT_PREFIX + name, new TypeReference<String>() {});
        if(!jwt.equals(token)) throw new CustomException.InvalidTokenException("token缓存过期，请重新登陆");
        // 刷新token有效期
        RedisUtil.set(JWT_PREFIX + name, jwt, JWT_EXPIRE_DURATION_MINUTES, TimeUnit.MINUTES);
    }
}
